...
\exe{difficulty=1}{
	...
	Montrer que le polynôme $x^2 + bx + c \in \Z[x]$ est réductible dans $\F_p[x]$ si et seulement si le discriminant $\Delta = b^2 - 4c$ est un carré parfait modulo $p$.
	...
	\begin{align*}
		...
		(2x+b)^2 = \Delta
	\end{align*}
	Nous avons utilisé ici que $p\neq2$ pour avoir que $4$ est inversible dans $\F_p$.
...
\dfn{degré}{
	...
	\begin{itemize}
		...
		$\deg(f) = d$ si $a_d \neq 0$ ; ou
		\item
		$\deg(f) = \minfty$ si $f = 0$ et le polynôme nul.
	\end{itemize}
}{dfn:degre}
...
\prop{}{
	...
	\begin{enumerate}
		...
		$\deg(fg) = \deg(f) + \deg(g)$
		\item
		Si $\K$ est de caractéristique nulle, alors
...
\exe{}{
	Montrer que l'identité $\deg(f+g) = \max\{\deg(f) ; \deg(g)\}$ n'est pas vraie dans $\F_p[x]$ ($p$ premier).
}{exe:deg-sum-Fp}{
	Prendre $f(x) = 1$ et $g(x) = p-1$.
	Alors $\deg(f) = \deg(g) = 1$ mais $\deg(f+g) = \minfty$.
}
...
\nt{
	...
	L'anneau des \emphindex{entiers de Gauss} $\Z[i]$ est euclidien avec la norme $|·|$ car c'est un réseau de $\C$ suffisamment fin pour que tout élément de $\Q[i]$ admette un élément de $\Z[i]$ dans sa boule unité.
	
	Ainsi, en élargissant le réseau, il est naturel que l'anneau perde sa division euclidienne.
	Il s'avère alors que $\Z[i\sqrt2]$ est toujours euclidien, mais pas $\Z[i\sqrt3]$.
}
...
\pf{}{
	...
	\begin{enumerate}
		...
		\item $p$ est unitaire ; 
	\end{enumerate}
	\begin{enumerate}[label=\underline{Condition \arabic*.}, leftmargin=80pt]
		\item
...
\exe{}{
	Montrer que l'idéal $(2, x)$ de $\Z[x]$ n'est pas principal : il n'existe pas de polynômes $f\in\Z[x]$ tel que $(2, x) = (f)$.
...
\nt{
	...
	Par conséquent, 2 est un irréductible dans $\Z[x]$ (voir exercice \ref{exe:k-star} et la définition \ref{def:irreductible}).
	...
	Nous verrons que l'irréducibilité d'un polynôme de $\Z[x]$ est garantie par le fait qu'il ne puisse se factoriser par aucune constante.
	Ces polynômes ont donc des coefficients à PGCD 1, et sont dits \emphindex{primitifs}.
}
...
\lem{}{
	Chaque polynôme rationnel $f\in\Q[x]$ s'écrit \underline{de façon unique} comme
		\[ f(x) = c f_0(x), \]
	avec $f\in\Z[x]$ primitif et $c\in\Q^\times$.
...
\cor{}{
	Soit $f\in\Z[x]$ un polynôme à coefficients entiers.
	$f$ est irréductible dans $\Z[x]$ si et seulement si $f$ est irréductible dans $\Q[x]$.
}{cor:irr-Zx-Qx}
