...
\dfn{polynôme à coefficients dans $\K$}{
	Soit $d\in\N$. Un \emphindex{polynôme} à coefficients dans $\K$ est une expression du type
		\[ a_0 + a_1x + a_2x^2 + \dots + a_d x^d, \]
	où $a_d \neq 0$, $d$ est le \emphindex{degré} du polynôme, les $a_i\in\K$ sont ses \emphindex{coefficients}, et $x$ est une \emphindex{indéterminée} abstraite (pas nécessairement un nombre de $\K$ !).
}{dfn:polynome}
...
\thm{division euclidienne}{
	Soient $a, b\in\K[x]$ deux polynômes à coefficients dans $\K$ avec $b\neq0$.
	Il existe deux uniques polynômes $q, r\in\K[x]$ vérifiant
	\begin{enumerate}
		\item $a = bq + r$ ; et
		\item $\deg(r) < \deg(b)$
	\end{enumerate}
	C'est la \emphindex{division euclidienne} de $a$ par $b$.
}{thm:div-euclidienne}

\nt{
	Il suit naturellement que $b$ divise $a$ dans $\K[x]$ si et seulement si le reste $r$ de la division euclidienne de $a$ par $b$ est nul.
	
	Remarquons que si $b=0$, alors la division euclidienne de $a$ par $b$ soit n'existe pas (si $a\neq0$), soit n'est pas unique (si $a=0$).
}
...
\exe{}{
	...
	Sinon, $\deg(a) \geq \deg(b)$ et nous construisons $q$ en annulant le degré le plus haut de $a$ itérativement.
	
	\begin{enumerate}
		\item
		Initialiser $q = 0$.
		\item
		Tant que $\deg(a-bq) \geq \deg(b)$, 
		\item
		Ajouter un multiple de $x^{\deg(a-bq) - \deg(b)}$ à $q$ de telle sorte que le degré de $a-bq$ diminue d'au moins 1.
		\item
		Fin Tant que
		\item
		Renvoyer $q$ et $r=a-bq$.
	\end{enumerate}

	\underline{Unicité.}
	
	Si $a = bq + r = bq' + r'$ avec $\deg(r), \deg(r') < \deg(b)$, alors
		\[ b(q-q') = r' - r, \]
	et donc $q-q'=0$ car $\deg(r'-r) < \deg(b)$ et que $b\neq0$.
	Il suit que $q=q'$ et donc que $r=r'$.
}

\nt{
	Le concept d'\emphindex{anneau euclidien} se généralise sans peine en remplaçant la fonction degré par une fonction\footnote{appelée \emphindex{norme} ou  \emphindex{stathme}} sur l'anneau $R$ qui prend ses valeurs dans $\N$.
}
...
\lem{}{
	...
	L'idéal $(a, b)$ est égal à l'idéal $(b, r)$, où $r$ est le reste de la division euclidienne de $a$ par $b$.
}{lem:div-euclid-ideal}

\pf{}{
	\underline{Inclusion $(a,b) \subseteq (b,r)$.}
	
	Si $f\in(a,b)$, alors $f = ua + vb$ pour certains $u, v\in\K$.
	Par suite, $f = u(qb+r) + vb = (uq+v)b + ur \in (b,r)$.
	
	\underline{Inclusion $(b,r) \subseteq (a,b)$.}
	
	Si $f\in(b,r)$, alors $f = ub + vr$ pour certains $u, v\in\K$.
	Par suite, $f = ub + v(a-bq) = va + (u-qv)b \in (a,b)$.
}

\nt{
	En réécrivant l'idéal $(a,b)$ comme l'idéal $(b,r)$, le deuxième polynôme a diminué en degré car $\deg(r) < \deg(b)$.
	Il est donc naturel de répéter le processus en calculant la division euclidienne de $b$ par $r$, afin de réduire le degré du deuxième polynôme encore davantage.
	Ce processus se terminera (en temps fini) alors nécessairement par un reste nul car les degrés formeront une suite d'entiers naturels strictement décroissante.
}

\dfn{suite des restes et quotients}{
	Soient $a_0, a_1 \in\K[x]$. 
	La \emphindex{suite des restes} est la suite de polynôme $a_0, a_1, \dots, a_k \in\K[x]$ où, pour $i\geq0$,  
	\begin{enumerate}
		\item
		$a_{i+2}$, est le reste non nul de la division euclidienne de $a_{i}$ par $a_{i+1}$ ; et
		\item
		$a_{k+1} = 0$.
	\end{enumerate}
	Le polynôme $a_k$ est donc le dernier reste non nul.
	
	La \emphindex{suite des quotients} est la suite de polynôme $q_1, q_2, \dots, q_k \in\K[x]$ où, pour $i\geq0$,
		\[ a_{i} = q_{i+1} a_{i+1} + a_i. \]
}{dfn:suite-restes}

\nt{
	%La suite des restes et quotients est finie car les degrés des restes forment une suite d'entiers naturels strictement décroissante.
	%
	L'\emphindex{algorithme d'Euclide étendu} suivant démontre l'existence du PGCD en plus de fournir une procédure permettant de le calculer.
}

\thm{algorithme d'Euclide étendu}{
	Soient $a_0, a_1 \in\K[x]$.
	Alors 
	\begin{enumerate}
		\item 
		le dernier reste non nul $a_k$ est un multiple constant de $\pgcd(a_0, a_1)$ ; et
		\item
		nous avons l'identité matricielle
		\begin{align*}
			\left[\begin{pmatrix}0&1\\1&-q_{k}\end{pmatrix}\cdots\begin{pmatrix}0&1\\1&-q_2\end{pmatrix}\begin{pmatrix}0&1\\1&-q_1\end{pmatrix}\right]\begin{pmatrix}a_{0}\\a_{1}\end{pmatrix}=\begin{pmatrix}a_k\\0\end{pmatrix}
		\end{align*}
	\end{enumerate}
}{thm:alg-Euclide}

\pf{}{
	D'après le lemme \ref{lem:div-euclid-ideal}, la suite de restes fournit les égalités d'idéaux suivantes.
		\[ (a_0, a_1) = (a_1, a_2) = (a_2, a_3) = \dots = (a_{k-1}, a_k) = (a_k, 0) = (a_k) \]
	
	Démontrons désormais que $p = c\cdot a_k$ est un PGCD de $a_0$ et $a_1$, où $c\in\K$ sert à rendre $c\cdot a_k$ unitaire.
	C'est-à-dire, $p$ vérifie les trois conditions suivantes.
	\begin{enumerate}
	...
	\begin{enumerate}[label=\underline{Condition \arabic*.}]
		\item
		L'égalité $(a_0, a_1) = (a_k) = (p)$ implique que $a_0, a_1 \in (p)$ et donc que $p$ divise à la fois $a_0$ et $a_1$.
		\item
		L'égalité $(a_0, a_1) = (a_k) = (p)$ implique que $p\in(a_0, a_1)$ est une combinaison linéaire de $a_0$ et $a_1$.
		D'après l'exercice \ref{exe:d-div-comb-lin}, un diviseur $d\in\K[x]$ de $a_0$ et de $a_1$ divise donc $p$.
		\item
		Vérifiée par construction de $p$.
	\end{enumerate}
	
	L'identité matricielle découle de la relation 
	\begin{align*}
		a_{i} = q_{i+1} a_{i+1} + a_{i+2} && \iff && \begin{pmatrix}0&1\\1&-q_{i+1}\end{pmatrix}\begin{pmatrix}a_{i}\\a_{i+1}\end{pmatrix}=\begin{pmatrix}a_{i+1}\\a_{i+2}\end{pmatrix}.
	\end{align*}
...
\ex{}{
	Nous cherchons à calculer le PGCD des polynômes $a_0(x) = x^5 + x^4 + x^3 + x^2 + x + 1$ et $a_1(x) = x^4-2x^3+3x^2-x-7$.
	La première division euclidienne donne
		\begin{align*}
			a_0 = q_1a_1 + a_2, && q_1(x) = x+3, && a_2(x) = 4x^3-7x^2+11x+22.
		\end{align*}
	La deuxième division euclidienne donne
		\begin{align*}
			a_1 = q_2a_2 + a_3 && q_2(x) = \frac1{16}(4x-1) && a_3(x) = -\frac3{16}x^2 - \frac{93}{16}x - \frac{45}8.
		\end{align*}
	Pour simplifier la troisième division euclidienne, il peut être utile de normaliser le multiplier le diviseur par une constante intégrée au quotient. Ici, multiplier $a_3$ par $16$ semble pertinent. La simple relation $a_2 = q_3 (16a_3) + a_4 = a_2 = (16q_3)a_3 + a_4$ concluera.
	La troisième division euclidienne donne alors
		\begin{align*}
			a_2 = q_3a_3 + a_4 && q_3(x) = -\frac{16}3(4x - 131) && a_4(x) = 3952x + 3952.
		\end{align*}
	Ici, il semble préférable de diviser $a_4$ par $3952$ pour simplifier les calculs.
	L'équivalence $16a_3 = q_4(a_4/3952) + a_5' \iff a_3 = (1/16/3952 q_4)a_4 + a_5'/16$ aboutit à
		\begin{align*}
			a_3 = q_4a_4 + a_5 && q_4(x) = -\frac{1}{16\times3952}(3x+90) && a_5(x) = 0.
		\end{align*}
	Nous concluons que le dernier reste non nul est $a_4(x) = 3952x+3952$, multiple de $x+1$, et donc que
		\[ \pgcd(a_0, a_1) = x+1. \]
	La relation matricielle s'écrit $M\cdot \begin{pmatrix}a_0\\a_1\end{pmatrix} = \begin{pmatrix}a_4\\0\end{pmatrix}$, avec
		\begin{align*}
			M &= 
			\begin{pmatrix}0&1\\1&-\frac{16}{3952}(3x+90)\end{pmatrix}
			\begin{pmatrix}0&1\\1&- \frac{16}3(4x - 131) \end{pmatrix}
			\begin{pmatrix}0&1\\1&- \frac1{16}(4x-1)\end{pmatrix}
			\begin{pmatrix}0&1\\1&- (x+3)\end{pmatrix}
			\\ &= 
			\frac{16}{3} \begin{pmatrix} -x^3 + 33x - 8 & x^3 - 30x^2 - 87x - 107 \\ (*) & (*) \end{pmatrix}
		\end{align*}
	La première ligne suffit à déduire l'\emphindex{identité de Bézout} $a_0u + a_1v = \pgcd(a_0,a_1)$ :
		\begin{align*}
		a_0 \underbrace{\left[\frac{16}3(-x^3 + 33x - 8)\right]}_{u} + a_1\underbrace{\left[\frac{16}3 (x^3 - 30x^2 - 87x - 107)\right]}_{v}  &= (x+1).
		\end{align*}
}{ex:Euclide-etendu}

\exe{}{
	Trouver $f, g \in\Q[x]$ avec $\deg(f), \deg(g) > 2$ et tels que $\pgcd(f, g) = x^2 +1$.
	Calculer les polynômes $u, v \in \Q[x]$ de l'identité de Bézout.
}{exe:Euclide-pgcd-forced}{
	todo
}

\nt{
	L'algorithme d'Euclide admet comme corollaire direct le \emphindex{théorème de Bachet-Bézout} suivant, d'abord énoncé dans les entiers relatifs par Claude-Gaspard Bachet de Méziriac en 1624, puis dans les polynômes par Étienne Bézout au 18ème siècle.
}

\cor{de Bachet-Bézout}{
	Soient $a, b \in\K[x]$. Alors
		\[ (a, b) = \bigpar{ \pgcd(a, b) }. \]
...
\exe{}{
	...
	Montrer que $f$ et $g$ sont premiers entre eux.
}{exe:irr-coprime}{
	todo
}

\exe{}{
	Montrer que $f(x) = x^3 + 2x + 1$ et $g(x) = x^2 +1$ sont premiers entre eux dans $\Q[x]$ et trouver les polynômes $u, v \in \Q[x]$ de l'identité de Bézout.
}{exe:Euclide-etendu}{
	todo
}
...
\pf{}{
	...
	Alors $\pgcd(p,a) = 1$ et le théorème de Bézout \ref{cor:Bezout} implique l'existence de $u, v\in\K[x]$ vérifiant
		\[ pu + av = 1. \]
...
\exe{difficulty=1}{
	...
	Le lemme d'Euclide \ref{lem:Euclide} implique donc $g|q$ et donc $f= qg = q'g^2$.
...
\exe{}{
	Montrer qu'il n'existe pas de drapeau strict infini d'idéaux principaux de $\K[x]$ :
		\[ I_1 \lhd I_2 \lhd I_3 \lhd \cdots \lhd \K[x]. \]
	L'anneau $\K[x]$ vérifie donc la \emphindex{condition de chaîne ascendante sur les idéaux principaux}.
	Cette condition garantit qu'un élément non nul se décompose en facteurs irréductibles, le seul danger de cette décomposition étant de ne jamais se terminer.
}{exe:Kx-finite-ideal-flag}{
	Supposons l'existence d'un tel drapeau. Le deuxième idéal n'est pas trivial, et $I_2 = (f)$ pour un certain polynômes $f\in\K[x]$ non nul.
	Comme 
	\begin{align*}
		\{ 0 \} \neq (f) \lhd (g) \text{ stricte} && \implies && g |f \text{ propre} && \implies \deg(g) < \deg(f),
	\end{align*}
	où nous avons écarté le cas $f=0$ pour obtenir l'inégalité large des degrés, et où l'inclusion stricte donne la division propre et l'inégalité stricte des degrés.
	Il suit donc que le drapeau ne peut contenir que $\deg(f) < \infty$ idéaux, le degré d'un polynôme non nul étant nécessairement un entier naturel.
}
...
\nt{
	...
	Certaines propositions utiliseront la notation $\pm$, lue « \emphindex{plus ou moins} », pour cette raison.
	L'exercice \ref{exe:primitive-divisors} est un exemple.
}
...
\exe{difficulty=1}{
	Soit $g_0\in\Z[x]$ un polynôme primitif.
	Montrer que si $f$ divise $g_0$ dans $\Z[x]$, alors $\pm f$ est également primitif.
}{exe:primitive-divisors}{
	Notons $g_0 = fq$ la division dans $\Z[x]$ ainsi que $f = c_f f_0$ la factorisation en polynôme primitif du lemme \ref{lem:primitif}.
	Le contenu $c_f$ est un entier relatif car $f$ appartient à $\Z[x]$.
	
	%Si un premier $p$ divise $c_f$, alors $p$ divisera $g_0$ ce qui contredit son caractère primitif.
	Alors $g_0 = c_f (f_0 q)$ est une relation dans $\Z[x]$ qui implique que $c_f$ divise $g_0$ et donc $1$ car ce dernier est primitif.
	Il suit que $c_f = \pm1$ et donc que $f = \pm f_0$.
%	Notons $g_0 = fq$ la division dans $\Z[x]$ ainsi que $f = c_f f_0$ et $q = c_q q_0$ la factorisation en polynômes primitifs du lemme \ref{lem:primitif}.
%	Les contenus $c_f$ et $c_q$ sont tous les deux des entiers relatifs, et le produit $f_0 q_0$ est primitif d'apèrs le lemme de Gauss \ref{lem:Gauss}.
%	
%	Par conséquent, nous avons l'implication suivante
%		\begin{align*} g_0 = fq && \iff g_0 = (c_f c_q) f_0 q_0 \implies 1 = c_f c_q, \end{align*}
%	équation dans $\Z$ qui implique $c_f = c_q = \pm 1$ et qui conclut.
%	
%	Un autre preuve est la suivante.
}

\subsection{Classification des irréductibles}

Le \emphindex{lemme de Gauss} suivant permet de classifier les polynômes irréductibles de $\Z[x]$.
...
\pf{}{\leavevmode
	\begin{enumerate}
		...
		le contenu d'un polynôme étant dans $\Z$ si et seulement si celui-ci est dans $\Z[x]$, ce qui concluera.
...
\exe{}{
	...
	\begin{enumerate}[itemsep=10pt]
		...
		\item
		Soient $f, g\in\K[x]$. Alors $\deg(fg) = \deg(f)+\deg(g)$.
		\item
		Un polynôme constant de $\K[x]$ est de degré nul.
		\item
		$7x$ divise $x$ dans $\C[x]$.
		\item
		$x+1$ divise $2x^5 -x^4 - 3x^3 + x^2 -3x - 4$ dans $\Z[x]$.
		\item
		$2(x-1)$ divise $2x^3 + 2x^2 - 5x +1$ dans $\Z[x]$.
	\end{enumerate}
}{exe:TF-polynomes}{
	\begin{enumerate}
		\item
		...
		\item
		Vrai (proposition \ref{prop:deg-div}).
		\item
		Faux à cause du polynôme nul de degré $\minfty$ par convention.
		\item
		Vrai car $(7x)\cdot\frac17 = x$.
		C'est faux dans $\Z$, en revanche !
		\item
		Vrai car $-1$ est racine du deuxième polynôme et que le premier est primitif.
		\item
		Faux. Le premier polynôme divise le deuxième dans $\Q[x]$ car $1$ est racine du deuxième.
		En revanche, dans $\Z[x]$, le deuxième polynôme est primitif mais pas le premier.
		La division dans $\Z[x]$ n'est donc pas possible, d'après l'exercice \ref{exe:primitive-divisors}.
	\end{enumerate}
...
\exe{difficulty=1}{
	Soit $p\in\K[x]$ un polynôme \emphindex{premier}, c'est-à-dire non nul et vérifiant, pour tout $a, b\in\K[x]$, $p|(ab) \implies p|a$ ou $p|b$.
...
\exe{difficulty=2}{
	...
	%Comme $f$ est à coefficients entiers et non constant, le lemme de Gauss \ref{lem:Gauss} implique qu'il suffit de montrer l'irréducibilité dans $\Z[x]$.
	Supposons, par contradiction, que $f$ soit réductible dans $\Q[x]$.
	Comme $f$ est à coefficients entiers, le lemme de Gauss \ref{lem:Gauss} implique que la réduction de $f$ peut être restreinte à $\Z[x]$.
	C'est-à-dire $f = ab$ avec $a, b\Z[x]$ non constants et de coefficients dominants non divisibles par $p$.
	...
	Ainsi, les deux facteurs $a$ et $b$ sont congrus à un multiple d'une puissance de $x$ modulo $p$, non constante car $a$ et $b$ sont non constants et de coefficients dominants non divisibles par $p$.
...
\exe{difficulty=1}{
	...
	\begin{enumerate}
		\item
		Si $f$ est réductible dans $\Q[x]$, alors, comme $f$ est unitaire (donc primitif) de $\Z[x]$, le lemme de Gauss (polynômes) \ref{lem:Gauss} garantit que $f$ est réductible dans $\Z[x]$.
		\item
