...
\exe{difficulty=1}{
	...
	\begin{enumerate}
		...
		\item
		constant de $\left(17 + 17320x - 10^{31}x^2 - \frac13 x^7\right)\left(41x^2 + 76x - 2\right)$ ;
		\item
		en $x^6$ de $(1+x)^7$.
	\end{enumerate}
	%\end{multicols}
	\emph{Il est possible de vérifier ses résultats à l'aide de WolframAlpha (\href{https://www.wolframalpha.com/}{https://www.wolframalpha.com/}).}
}{exe:coeff-reading-3}{
	todo
}
...
\exe{}{
	Est-ce que la famille de polynômes $\{ x^2+4x - 2 ; x+2 ; 50 \} \subseteq\R[x]$ est linéairement indépendante sur $\R$ ? 
...
\exe{}{
	Est-ce que la famille de polynômes $\{ x+1 ; x+2 \} \subseteq\R[x]$ est libre sur $\R$ ?
}{exe:lin-indep1}{
	todo
}

\exe{}{
	Est-ce que la famille de polynômes $\{ 2x^2+4x - 2 ; x^2 + x + 1 ; x-2 \} \subseteq\R[x]$ est libre sur $\R$ ?
}{exe:lin-indep3}{
...
\exe{}{
	Vrai ou faux ? Le degré d'un polynôme $f$ est noté $\deg(f)$.
...
\exe{difficulty=1}{
	...
	\end{enumerate}
	\end{multicols}
	En déduire que $x+1$ divise $x^8 - 1$ et que $x$ divise $(x+1)^{1204} - 1$.
