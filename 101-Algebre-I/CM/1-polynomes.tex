...
\dfn{indépendance linéaire}{
	Soit $d\in\N$. 
	Les monômes $1, x, x^2, \dots, x^d$ sont \emphindex{linéairement indépendants} car dès que
		\[ \alpha_0 + \alpha_1 x + \alpha_2 x^2 + \dots + \alpha_d x^d = 0 \]
	pour certains nombres réels $\alpha_0,\alpha_1, \alpha_2 \dots, \alpha_d$, alors
...
\nt{
	Les polynômes $\{ 1 ; x ; x^2 ; \dots \}$ sont indépendants par définition.
...
\exe{difficulty=1}{
	Montrer que la famille de fonctions polynomiales $\{1 ; x\}$ est linéairement indépendante. 
	C'est-à-dire que dès que $\alpha_0 + \alpha_1 x$ est la fonction nulle pour certains nombres réels $\alpha_0, \alpha_1 \in\R$, alors $\alpha_0 = \alpha_1 = 0$.
}{exe:1x-indep}{
	todo
}
...
\exe{difficulty=2}{
	Soit $d\in\N$. 
	Montrer que la famille de fonctions polynomiales $\{1 ; x ; \dots ; x^d\}$ est linéairement indépendante.
}{exe:1xxd-indep}{
	todo
}
