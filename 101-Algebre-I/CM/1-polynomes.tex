...
\dfn{polynôme à coefficients réels, fonction polynomiale}{
	...
	où $a_d \neq 0$, $d$ est le \emphindex{degré} du polynôme, les $a_i\in\K$ sont ses \emphindex{coefficients}, et $x$ est une \emphindex{indéterminée} abstraite (pas nécessairement un nombre réel !).
	
	Une fonction $f(x) = a_0 + a_1x + a_2x^2 + \dots + a_d x^d$ de $\R$ dans $\R$ est une \emphindex{fonction polynomiale}.
}{dfn:polynome}

\nomen{
	Le \emphindex{polynôme nul} est $0$, le polynôme à coefficients tous nuls.

	La \emphindex{fonction nulle sur $\R$} est la fonction $f$ vérifiant $f(x) = 0$ pour tout $x\in\R$.
}
...
\nt{
	Nous pourrions définir l'addition et la multiplication des polynômes différemment, mais nous perdrions les identités $(f+g)(a) = f(a) + g(a)$ et $(f\cdot g)(a) = f(a)g(a)$ lors de l'évaluation des polynômes en un objet $a$ sujet à l'axiome de distributivité.
}

\ex{}{
	Soient $f(x) = 2x+1$ et $g(x) = x^2 - 8$.
	Alors
		\begin{align*}
			(f+g)(x) = x^2 + 2x - 7, && \et && (f\cdot g)(x) = (2x+1)(x^2 -8) = 2x^3 + x^2 -16x - 8.
		\end{align*}
}{ex:add-mult-poly}
...
\nomen{
	...
	Une \emphindex{combinaison linéaire} sur $\R$ est une expression de la forme
		\[ \alpha_1 p_1 + \alpha_2 p_2 + \dots + \alpha_k p_k, \]
	où les $\alpha_i\in\R$ sont des nombres réels pour $i=1, 2, \dots, k$.
...
\nt{
	Le $0$ à droite de l'égalité $\alpha_1 p_1 + \alpha_2 p_2 + \dots + \alpha_k p_k = 0$ correspond au polynôme nul dont tous les coefficients sont nuls.
}

\ex{}{
	Considérons $\{ x+1 ; x^2 + 3 ; x ; 2 \}$ une famille de polynômes.
	Alors
		\[ 2(x+1) - (x^2 + 3) + 2x - 3(2) = -x^2 + 4x -7 \]
	est une combinaison linéaire non triviale de ces polynômes.
	
	La combinaison
		\[ 3(x+1) + 0(x^2 +3) - 3x - \frac32(2) = 0 \]
	est une combinaison nulle non triviale.
}{ex:comb-lin}
...
\nomen{
	Une famille linéairement indépendante est dite \emphindex{famille libre}.

	Une famille linéairement dépendante est dite \emphindex{famille liée}.
}

\ex{}{
	L'exemple \ref{ex:comb-lin} montre que la famille de polynômes $\{ x+1 ; x^2 + 3 ; x ; 2 \}$ est liée car il existe une combinaison linéaire nulle et non triviale.
	
	En enlevant le premier polynôme, la famille $\{ x+1 ; x^2 + 3 ; 2 \}$ devient libre.
	En effet, si 
		\[ \alpha_1 (x+1) + \alpha_2 (x^2 + 3) + \alpha_3 (2) = 0 \]
	avec $\alpha_1, \alpha_2, \alpha_3\in\R$, alors
		\[ \alpha_2 x^2 + \alpha_1 x + (\alpha_1 + 3\alpha_2 + 2\alpha_3) 1 = 0. \]
	Par indépendant linéaire des monômes $\{ 1 ; x ; x^2 \}$, il suit que
		\[ \alpha_2 = \alpha_1 = \alpha_1 + 3\alpha_2 + 2\alpha_3 = 0. \]
	Par conséquent, $\alpha_1=\alpha_2=\alpha_3=0$ et l'unique combinaison linéaire nulle est la combinaison triviale.
}{ex:indep-lin}
...
\prop{}{
	Soient $f, g\in\R[x]$. Alors, avec la convention $\deg(0) = \minfty$,
	\begin{enumerate}
		\item
		$\deg(fg) = \deg(f) + \deg(g)$
		\item
...
\dfn{racine, zéro}{
	...
	Un nombre réel $\alpha\in \R$ est une \emphindex{racine} du polynôme $f$ ou \emphindex{zéro} de la fonction polynomiale $f$ dès que $f(\alpha) = 0$.
}{dfn:racine}
...
\prop{}{
	Soit $f\in\R[x]$ un polynôme non nul à coefficient réels et $\alpha\in\R$ une racine.
	Alors le monôme $x-\alpha$ divise $f$ dans $\R[x]$ : il existe un $q\in\R[x]$ non nul vérifiant
		\[ f(x) = (x-\alpha) q(x). \]
}{prop:x-r-div}

\pf{}{
	Soit $f(x) = (x-\alpha)q(x) + c(x)$, $\deg(c) < \deg(x-\alpha)$ la division euclidienne de $x-\alpha$ par $f$.
	\begin{enumerate}
		\item
		...
		\item
		L'évaluation en $x=\alpha$ aboutit à $f(\alpha) = q(r) (\alpha-\alpha) + c \iff 0 = c$.
	\end{enumerate}
	Par conséquent, $f(x) = (x-\alpha) q(x)$.
}
...
\dfn{multiplicité}{
	Soit $f\in\R[x]$ un polynôme à coefficients réels et $\alpha\in\R$ une racine.
	La \emphindex{multiplicité} $m\in\N^*$ de la racine $\alpha$ est l'entier tel que $(x-\alpha)^{m}$ divise $f$ dans $\R[x]$ mais pas $(x-\alpha)^{m+1}$.
}{dfn:multiplicite}
...
\cor{}{
	...
	Alors $f$ se \emphindex{factorise} comme
		\[ f(x) = \left(\prod_{\text{$\alpha$ racine de $f$}} (x-\alpha)^{m(\alpha)}\right) \cdot q(x), \]
...
\prop{}{
	...
	Montrer que $\alpha$ est une racine de multiplicité $m$ si et seulement si $f(\alpha) = f'(\alpha) = \cdots = f^{(m-1)}(\alpha) = 0$ et $f^{(m)}(\alpha) \neq 0$.
}{prop:mult-from-diff}
...
\exe{difficulty=1}{
	Démontrer le corollaire \ref{cor:derivative-test}.
}{exe:derivative-test}{
	\underline{Direction $\implies$.}
	
	Soit $\alpha\in\R$ une racine de $f$ de multiplicité $m\in\N^*$.
	Comme $x-\alpha$ divise $f$ mais pas $f'$, nous avons $f(\alpha) = 0$ et $f'(\alpha) \neq 0$.
	...
	Le polynôme $d$ n'étant pas constant, il existe un réel $\alpha\in\R$ tel que $x-\alpha$ divise $d$ et donc $f$ et $f'$.
	Ainsi $f(\alpha) = f'(\alpha) = 0$ et la racine $\alpha$ n'est pas simple.
}
...
\exe{difficulty=1}{
	Considérons $f(x) = a_0 + a_1x + \dots + a_nx^n$ un polynôme à coefficients réels non nuls et dont les signes s'alternent : lorsqu'un l'un est positif, le suivant est négatif, et vice-versa.

	Montrer que les racines réelles de $f$ sont soit toutes strictement positives, soit toutes strictement négatives.
}{exe:no-negative-roots}{
	todo
}
